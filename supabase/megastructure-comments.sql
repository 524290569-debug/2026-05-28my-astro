...
create table if not exists public.megastructure_sensitive_terms (
  id uuid primary key default gen_random_uuid(),
  category text not null,
  term text not null,
  normalized_term text not null,
  action text not null default 'pending' check (action in ('pending', 'rejected')),
  severity numeric(5,2) not null default 0.75,
  source_file text,
  enabled boolean not null default true,
  created_at timestamptz not null default now()
);

create unique index if not exists megastructure_sensitive_terms_category_term_unique
on public.megastructure_sensitive_terms (category, normalized_term);

create index if not exists megastructure_sensitive_terms_enabled_idx
on public.megastructure_sensitive_terms (enabled, action, severity desc);
...
comment on table public.megastructure_sensitive_terms is '巨构主义评论敏感词规则表，由本地敏感词库导入，供数据库自动审核触发器使用。';
comment on column public.megastructure_sensitive_terms.id is '敏感词规则 ID。';
comment on column public.megastructure_sensitive_terms.category is '敏感词分类，通常来自词库文件名。';
comment on column public.megastructure_sensitive_terms.term is '敏感词原文。';
comment on column public.megastructure_sensitive_terms.normalized_term is '归一化后的匹配词，通常为小写和去除首尾空白后的文本。';
comment on column public.megastructure_sensitive_terms.action is '命中后的处理动作，pending 表示转人工审核，rejected 表示自动拒绝。';
comment on column public.megastructure_sensitive_terms.severity is '规则风险分，0 表示低风险，1 表示高风险。';
comment on column public.megastructure_sensitive_terms.source_file is '敏感词来源文件名。';
comment on column public.megastructure_sensitive_terms.enabled is '规则是否启用。';
comment on column public.megastructure_sensitive_terms.created_at is '敏感词规则导入时间。';
...
alter table public.megastructure_sensitive_terms enable row level security;
...
grant select on public.megastructure_sensitive_terms to authenticated;
...
begin
  ...
  select
    ...
    severity,
    normalized_term
  into matched_sensitive
  from public.megastructure_sensitive_terms
  where enabled = true
    and char_length(normalized_term) >= 2
    and position(normalized_term in normalized_body) > 0
  order by
    case action when 'rejected' then 1 else 0 end desc,
    severity desc,
    char_length(normalized_term) desc
  limit 1;

  if found then
    labels := labels || jsonb_build_array('sensitive_dictionary', 'sensitive:' || matched_sensitive.category);

    if matched_sensitive.action = 'rejected' then
      target_status := 'rejected';
      target_reason := '命中敏感词库拒绝规则：' || matched_sensitive.category || '。';
      target_score := greatest(target_score, matched_sensitive.severity);
    elsif target_status = 'approved' then
      target_status := 'pending';
      target_reason := '命中敏感词库待审规则：' || matched_sensitive.category || '。';
      target_score := greatest(target_score, matched_sensitive.severity);
    else
      target_reason := target_reason || ' 同时命中敏感词库：' || matched_sensitive.category || '。';
      target_score := greatest(target_score, matched_sensitive.severity);
    end if;
  end if;
...
drop policy if exists "admins read sensitive terms" on public.megastructure_sensitive_terms;
create policy "admins read sensitive terms"
on public.megastructure_sensitive_terms for select
to authenticated
